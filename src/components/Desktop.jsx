...
const Desktop = () => {
  ...
  const loadDesktopSettings = () => {
    if (typeof window !== "undefined") {
      const savedWallpaper =
        localStorage.getItem("desktopWallpaper") || "#54a8a8";
  ...
  const [desktopWallpaper, setDesktopWallpaper] = useState(
    savedDesktopSettings.wallpaper
  );
  ...
  const completeIconPositions = useMemo(() => {
    const desktopServices = services.filter(
      (service) => service.id !== "display-properties"
    );
  ...
  const createOnOpenMixerCallback = useCallback(
    (musicMakerServiceId) => {
      return () => {
        console.log(
          "onOpenMixer callback called for service:",
          musicMakerServiceId
        );
        setOpenWindows((prev) => {
          // Always find the window dynamically by serviceId - don't rely on ref or closure
          const musicMakerWindow = prev.find(
            (w) => w.serviceId === musicMakerServiceId && w.content.isMusicMaker
          );

          if (!musicMakerWindow) {
            console.warn(
              "Music Maker window not found for service:",
              musicMakerServiceId
            );
            console.log(
              "Available windows:",
              prev.map((w) => ({
                ...
                isMusicMaker: w.content?.isMusicMaker,
              }))
            );
            return prev;
          }
          ...
          musicMakerWindowIdsRef.current.set(
            musicMakerServiceId,
            currentWindowId
          );
          console.log("Found Music Maker window:", currentWindowId);

          const existingMixer = prev.find(
            (w) => w.parentWindowId === currentWindowId && w.content.isMixer
          );
          ...
          if (existingMixer) {
            ...
            const mixerSynthVolume =
              musicMakerWindow.content.synthVolume ?? 0.7;
                ...
                setOpenWindows((prevWindows) =>
                  prevWindows.map((w) =>
                    w.id === currentWindowId
                      ...
                      : w.parentWindowId === currentWindowId &&
                          w.content.isMixer
                        ? {
                            ...w,
                            content: { ...w.content, drumVolume: volume },
                          }
                        : w
                  )
                );
                  ...
                  prevWindows.map((w) =>
                    w.id === currentWindowId
                      ...
                      : w.parentWindowId === currentWindowId &&
                        ...
                        ? {
                            ...w,
                            content: { ...w.content, synthVolume: volume },
                          }
                        : w
                  )
                );
            ...
            const { isMusicMaker: _, ...musicMakerContentWithoutFlag } =
              musicMakerWindow.content;
            const mixerWindow = {
    ...
    const createReverbOpener = (channelName) => {
      return () => {
        setOpenWindows((prev) => {
          ...
          const existingReverb = prev.find(
            (w) =>
              w.parentWindowId === currentWindowId &&
              w.content.isReverb &&
              w.content.channelName === channelName
          );
          ...
          if (existingReverb) {
            ...
            const channelKeyPrefix =
              channelName === "drums" ? "drum" : channelName;
            ...
            const enabled =
              musicMakerWindow.content[enabledKey] ??
              musicMakerWindow.content.drumsReverbEnabled ??
              false;
            const roomSize =
              musicMakerWindow.content[roomSizeKey] ??
              musicMakerWindow.content.drumsReverbRoomSize ??
              0.5;
            const damping =
              musicMakerWindow.content[dampingKey] ??
              musicMakerWindow.content.drumsReverbDamping ??
              0.5;
            const wetLevel =
              musicMakerWindow.content[wetLevelKey] ??
              musicMakerWindow.content.drumsReverbWetLevel ??
              0.3;
            const dryLevel =
              musicMakerWindow.content[dryLevelKey] ??
              musicMakerWindow.content.drumsReverbDryLevel ??
              0.7;
                ...
                prevWindows.map((w) =>
                  w.id === currentWindowId
                    ...
                    : w.parentWindowId === currentWindowId && w.content.isMixer
                      ? {
                          ...w,
                          content: { ...w.content, [roomSizeKey]: value },
                        }
                      : w.id === reverbWindowId
                ...
                prevWindows.map((w) =>
                  w.id === currentWindowId
                    ...
                    : w.parentWindowId === currentWindowId && w.content.isMixer
                      ? {
                          ...w,
                          content: { ...w.content, [wetLevelKey]: value },
                        }
                      : w.id === reverbWindowId
                ...
                prevWindows.map((w) =>
                  w.id === currentWindowId
                    ...
                    : w.parentWindowId === currentWindowId && w.content.isMixer
                      ? {
                          ...w,
                          content: { ...w.content, [dryLevelKey]: value },
                        }
                      : w.id === reverbWindowId
    ...
    (service) => {
      ...
      setOpenWindows((prev) => {
        ...
        if (existingWindow) {
          ...
          if (
            service.content.isMusicMaker &&
            existingWindow.content.isMusicMaker
          ) {
            ...
            const freshMixerCallback =
              createOnOpenMixerCallback(musicMakerServiceId);
        ...
        const windowWidth = service.content.isPDF
          ? 800
          : service.content.isBrowser
            ? 900
            : service.content.isFolder
              ? 600
              : service.content.isMusicMaker
                ? 800
                : service.content.isContactForm
                  ? 700
                  : service.content.isSEOChecker
                    ? 900
                    : 500;
        const windowHeight = service.content.isPDF
          ? 600
          : service.content.isBrowser
            ? 700
            : service.content.isFolder
        ...
        const constrainedX = isMobile
          ? 0
          : Math.max(0, Math.min(initialX, viewportWidth - windowWidth));
        const constrainedY = isMobile
          ? 0
          : Math.max(0, Math.min(initialY, viewportHeight - windowHeight));
        const finalWidth = shouldMaximize
          ? viewportWidth
          : isMobile
            ? viewportWidth
            : windowWidth;
        const finalHeight = shouldMaximize
          ? viewportHeight
          : isMobile
            ? Math.min(windowHeight, viewportHeight)
            : windowHeight;
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? { ...w, content: { ...w.content, drumVolume: volume } }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? { ...w, content: { ...w.content, drumVolume: volume } }
                      : w
                )
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, drumReverbEnabled: enabled },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, drumReverbRoomSize: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, drumReverbDamping: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, drumReverbWetLevel: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, drumReverbDryLevel: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, synthReverbEnabled: enabled },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
                          ...
                          content: {
                            ...w.content,
                            synthReverbEnabled: enabled,
                          },
                        }
                      : w.parentWindowId === musicMakerWindowId &&
                          w.content.isReverb &&
                          w.content.channelName === "synth"
                        ? { ...w, content: { ...w.content, enabled } }
                        : w
                )
              );
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, synthReverbRoomSize: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, synthReverbDamping: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, synthReverbWetLevel: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, synthReverbDryLevel: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, masterReverbEnabled: enabled },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
                          ...
                          content: {
                            ...w.content,
                            masterReverbEnabled: enabled,
                          },
                        }
                      : w.parentWindowId === musicMakerWindowId &&
                          w.content.isReverb &&
                          w.content.channelName === "master"
                        ? { ...w, content: { ...w.content, enabled } }
                        : w
                )
              );
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, masterReverbRoomSize: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, masterReverbDamping: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, masterReverbWetLevel: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
              ...
              setOpenWindows((prevWindows) =>
                prevWindows.map((w) =>
                  w.id === musicMakerWindowId
                    ? {
                        ...w,
                        content: { ...w.content, masterReverbDryLevel: value },
                      }
                    : w.parentWindowId === musicMakerWindowId &&
                        w.content.isMixer
                      ? {
  ...
  const closeWindow = (windowId) => {
    ...
    setOpenWindows(
      openWindows.filter(
        (w) => w.id !== windowId && w.parentWindowId !== windowId
      )
    );
  };
    ...
    setOpenWindows((prev) =>
      prev.map((w) => {
        if (w.id === windowId) {
          ...
          const constrainedX = isMobile
            ? 0
            : Math.max(0, Math.min(x, viewportWidth - windowWidth));
          const constrainedY = isMobile
            ? 0
            : Math.max(0, Math.min(y, viewportHeight - windowHeight));
    ...
    setOpenWindows((prev) =>
      prev.map((w) => {
        if (w.id === windowId) {
          ...
          const minWidth = w.content.isPDF
            ? 600
            : w.content.isBrowser
              ? 600
              : w.content.isFolder
                ? 400
                : w.content.isMusicMaker
                  ? 600
                  : w.content.isContactForm
                    ? 500
                    : w.content.isSEOChecker
                      ? 900
                      : 400;
          const minHeight = w.content.isPDF
            ? 400
            : w.content.isBrowser
              ? 400
              : w.content.isFolder
  ...
  const constrainWindowPosition = (w) => {
    ...
    const constrainedX = Math.max(
      0,
      Math.min(w.x, viewportWidth - windowWidth)
    );
    const constrainedY = Math.max(
      0,
      Math.min(w.y, viewportHeight - windowHeight)
    );
  ...
  useEffect(() => {
    const handleResize = () => {
      setOpenWindows((prevWindows) => prevWindows.map(constrainWindowPosition));
    };
  ...
  const handleDesktopMouseDown = (e) => {
    ...
    if (
      ...
      (e.target.closest(".desktop-background") &&
      ...
      if (selectionStartRef.current && desktopRef.current) {
        ...
        openWindows.forEach((window) => {
          if (!window.minimized && !window.maximized) {
            ...
            if (
              x < windowRect.right &&
              x + width > windowRect.left &&
              y < windowRect.bottom &&
              y + height > windowRect.top
            ) {
        ...
        services
          .filter((service) => service.id !== "display-properties")
          .forEach((service) => {
            ...
            if (iconPos) {
              ...
              if (
                x < iconRect.right &&
                x + width > iconRect.left &&
                y < iconRect.bottom &&
                y + height > iconRect.top
              ) {
  ...
  const desktopStyle =
    desktopWallpaper === "custom" ? { backgroundColor: desktopColor } : {};
      ...
      {openWindows.map((window) => (
        <Window
          ...
          onResize={(width, height, x, y) =>
            updateWindowSize(window.id, width, height, x, y)
          }
